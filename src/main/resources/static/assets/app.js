...
})({"2dXcb":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = 1234;
var HMR_SECURE = false;
var HMR_ENV_HASH = "4a236f9275d0a351";
module.bundle.HMR_BUNDLE_ID = "202f2c3211191540";
"use strict";
...
},{}],"1IDVh":[function(require,module,exports) {
var _appCss = require("./app.css");
var _splideJs = require("./splide.js");

},{"./app.css":"c3ODh","./splide.js":"krjVW"}],"c3ODh":[function() {},{}],"krjVW":[function(require,module,exports) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    for(var i = 0; i < elms.length; i++)new _splideDefault.default(elms[i], {
        type: 'loop',
        perPage: 3,
        perMove: 1
    }).mount();
...
},{"@splidejs/splide":"jKkq4","@splidejs/splide/dist/css/splide.min.css":"4OZhZ","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"jKkq4":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
function find1(arrayLike, predicate) {
...
function children1(parent, selector) {
    return parent ? slice(parent.children).filter((child)=>matches(child, selector)
    ) : [];
}
function child1(parent, selector) {
    return selector ? children1(parent, selector)[0] : parent.firstElementChild;
}
...
function parseHtml(html) {
    return child1(new DOMParser().parseFromString(html, "text/html").body);
}
...
const { min , max: max1 , floor , ceil , abs  } = Math;
function approximatelyEqual(x, y, epsilon) {
    return abs(x - y) < epsilon;
}
function between(number, minOrMax, maxOrMin, exclusive) {
    const minimum = min(minOrMax, maxOrMin);
    const maximum = max1(minOrMax, maxOrMin);
    return exclusive ? minimum < number && number < maximum : minimum <= number && number <= maximum;
}
function clamp(number, x, y) {
    const minimum = min(x, y);
    const maximum = max1(x, y);
    return min(max1(minimum, number), maximum);
}
function sign(x) {
    return +(x > 0) - +(x < 0);
}
...
function Options(Splide2, Components2, options) {
    ...
    function observe() {
        const item = find1(points, (item2)=>item2[1].matches
        ) || [];
...
function Elements1(Splide2, Components2, options) {
    ...
    function collect() {
        slider = child1(root, `.${CLASS_SLIDER}`);
        track = query(root, `.${CLASS_TRACK}`);
        list = child1(track, `.${CLASS_LIST}`);
        assert(track && list, "A track/list element is missing.");
        push(slides, children1(list, `.${CLASS_SLIDE}:not(.${CLASS_CLONE})`));
    ...
    function find(selector) {
        return child1(root, selector) || child1(slider, selector);
    }
    function getClasses() {
        return [
...
function Slide$1(Splide2, index, slideIndex, slide) {
    ...
    const container = child1(slide, `.${CLASS_CONTAINER}`);
...
function Slides1(Splide2, Components2, options) {
...
function Layout(Splide2, Components2, options) {
    ...
    function getGap() {
        const Slide = getAt(0);
        return Slide && parseFloat(style1(Slide.slide, resolve("marginRight"))) || 0;
    }
...
function Move1(Splide2, Components2, options) {
    const { on , emit  } = EventInterface(Splide2);
...
function Controller1(Splide2, Components2, options) {
    const { on  } = EventInterface(Splide2);
...
function Arrows(Splide2, Components2, options) {
    ...
    function createArrows() {
        ...
        before(wrapper, child1(options.arrows === "slider" && Elements.slider || Splide2.root));
    }
...
function Cover(Splide2, Components2, options) {
    ...
    function apply(cover) {
        Components2.Slides.forEach((Slide)=>{
            const img = child1(Slide.container || Slide.slide, "img");
...
function Scroll1(Splide2, Components2, options) {
    const { on , emit  } = EventInterface(Splide2);
    const { Move  } = Components2;
    ...
    function computeDuration(distance) {
        return max1(distance / BASE_VELOCITY, MIN_DURATION);
    }
    function clear() {
    ...
    function onPointerUp(e) {
        ...
        if (lastEvent) {
            if (dragging || e.cancelable && isSliderDirection()) {
                ...
                else if (Splide2.is(FADE)) Controller.go(index + orient(sign(velocity)));
                else Controller.go(Controller.toDest(destination), true);
                prevent(e);
            }
            emit(EVENT_DRAGGED);
        } else if (!isFree && getPosition() !== Move.toPosition(index)) Controller.go(index, true);
        dragging = false;
    }
    function save(e) {
        ...
        Components2.Slides.forEach((_Slide)=>{
            queryAll(_Slide.slide, IMAGE_SELECTOR).forEach((_img)=>{
                ...
                if (src !== _img.src || srcset !== _img.srcset) {
                    ...
                    const _spinner = child1(parent, `.${className}`) || create("span", className, parent);
...
function Fade(Splide2, Components2, options) {
    ...
    function start(index, done) {
        const { track  } = Components2.Elements;
        style1(track, "height", unit(rect(track).height));
        nextTick(()=>{
            done();
            style1(track, "height", "");
        });
...
function Slide1(Splide2, Components2, options) {
    const { bind  } = EventInterface(Splide2);
    ...
    function apply(transition) {
        style1(list, "transition", transition);
    }
    return {
        mount,
        start,
...
const _Splide = class {
    ...
    mount(Extensions, Transition) {
        ...
        this._Transition = Transition || this._Transition || (this.is(FADE) ? Fade : Slide1);
    ...
    set options(options1) {
        const { _options  } = this;
        merge(_options, options1);
...
class Style {
    constructor(id, options3){
        this.styles = {
        };
        this.id = id;
        this.options = options3;
    }
    rule(selector1, prop1, value1, breakpoint1) {
        breakpoint1 = breakpoint1 || "default";
        const selectors = this.styles[breakpoint1] = this.styles[breakpoint1] || {
        };
        const styles = selectors[selector1] = selectors[selector1] || {
        };
        styles[prop1] = value1;
    }
    build() {
...
class SplideRenderer {
    constructor(contents, options2, config, defaults){
        this.slides = [];
        this.options = {
        };
        this.breakpoints = [];
        ...
        merge(merge(this.options, DEFAULTS), options2 || {
        });
    ...
    buildTranslate(options14) {
        const { resolve , orient  } = this.Direction;
        const values = [];
        values.push(this.cssOffsetClones(options14));
        values.push(this.cssOffsetGaps(options14));
        if (this.isCenter(options14)) {
            values.push(this.buildCssValue(orient(-50), "%"));
            values.push(...this.cssOffsetCenter(options14));
        }
        return values.filter(Boolean).map((value)=>`translate${resolve("X")}(${value})`
        ).join(" ");
    }
    cssOffsetClones(options4) {
        const { resolve , orient  } = this.Direction;
        const cloneCount = this.getCloneCount();
        if (this.isFixedWidth(options4)) {
            const { value , unit: unit2  } = this.parseCssValue(options4[resolve("fixedWidth")]);
            return this.buildCssValue(orient(value) * cloneCount, unit2);
        }
        const percent = 100 * cloneCount / options4.perPage;
        return `${orient(percent)}%`;
    }
    cssOffsetCenter(options5) {
        const { resolve , orient  } = this.Direction;
        if (this.isFixedWidth(options5)) {
            const { value , unit: unit2  } = this.parseCssValue(options5[resolve("fixedWidth")]);
            return [
                this.buildCssValue(orient(value / 2), unit2)
            ];
        }
        const values = [];
        const { perPage , gap  } = options5;
    ...
    cssOffsetGaps(options6) {
        const cloneCount = this.getCloneCount();
        if (cloneCount && options6.gap) {
            const { orient  } = this.Direction;
            const { value , unit: unit2  } = this.parseCssValue(options6.gap);
            if (this.isFixedWidth(options6)) return this.buildCssValue(orient(value * cloneCount), unit2);
            const { perPage  } = options6;
    ...
    cssPadding(options7, right) {
        const { padding  } = options7;
        const prop = this.Direction.resolve(right ? "right" : "left", true);
        return padding && unit(padding[prop] || (isObject(padding) ? 0 : padding)) || "0px";
    }
    cssTrackHeight(options8) {
        let height = "";
        if (this.isVertical()) {
            height = this.cssHeight(options8);
            assert(height, '"height" is missing.');
            height = `calc(${height} - ${this.cssPadding(options8, false)} - ${this.cssPadding(options8, true)})`;
        }
        return height;
    }
    cssHeight(options9) {
        return unit(options9.height);
    }
    cssSlideWidth(options10) {
        return options10.autoWidth ? "" : unit(options10.fixedWidth) || (this.isVertical() ? "" : this.cssSlideSize(options10));
    }
    cssSlideHeight(options11) {
        return unit(options11.fixedHeight) || (this.isVertical() ? options11.autoHeight ? "" : this.cssSlideSize(options11) : this.cssHeight(options11));
    }
    cssSlideSize(options12) {
        const gap = unit(options12.gap);
        return `calc((100%${gap && ` + ${gap}`})/${options12.perPage || 1}${gap && ` - ${gap}`})`;
    }
    cssAspectRatio(options13) {
        const { heightRatio  } = options13;
        return heightRatio ? `${1 / heightRatio}` : "";
    }
    buildCssValue(value3, unit2) {
        return `${value3}${unit2}`;
    }
    parseCssValue(value2) {
        if (isString(value2)) {
            const number = parseFloat(value2) || 0;
            const unit2 = value2.replace(/\d*(\.\d*)?/, "") || "px";
            return {
                value: number,
                unit: unit2
            };
    ...
    generateClones(contents1) {
        ...
        push(slides.slice(-count).reverse(), slides.slice(0, count)).forEach((content, index)=>{
            ...
            index < count ? contents1.unshift(clone) : contents1.push(clone);
        });
    }
    getCloneCount() {
        if (this.isLoop()) {
            ...
            const perPage = max1(...this.breakpoints.map(([, options2])=>options2.perPage
            ));
...
},{}],"4OZhZ":[function() {},{}]},["2dXcb","1IDVh"], "1IDVh", "parcelRequire5a08")
